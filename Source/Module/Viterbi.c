...
static int hamming_distance(const int expected[N_OUT],
                            const uint8_t received[4], int pos) {
  int d = 0;
...
static void deinterleave(const unsigned char *in, unsigned char *out) {
  ...
  for (int r = 0; r < 8; r++) {
    for (int c = 0; c < 4; c++) {
      out_bits[TOTAL_BITS - 1 - (4 * r + c)] =
          in_bits[TOTAL_BITS - 1 - (r + c * 8)];
    }
  }
...
static void viterbi_decode_hard(const uint8_t received[4], uint8_t decoded[2],
                                int traceback_len) {
#define MAX_STATES 4
#define SYMBOLS 16
